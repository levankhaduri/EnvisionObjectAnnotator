...
def calculate_basic_score(image_path: Path) -> Dict[str, float]:
    ...
    combined = (
        0.4 * min(sharpness / 100.0, 1.0)
        + 0.2 * brightness
        + 0.2 * edge_density
        + 0.2 * min(color_variance / 50.0, 1.0)
    )
...
def sample_frames_evenly(
    frames_dir: Path, max_samples: int = 50
) -> List[Tuple[int, Path]]:
...
def check_dinov2_available() -> bool:
    """Check if DINOv2 can be loaded (requires GPU)."""
    try:
        import torch
...
def load_dinov2():
    """Load DINOv2 model onto best available device."""
    import torch
...
def calculate_dinov2_score(
    image_path: Path, model, device
) -> Tuple[float, np.ndarray]:
    """Calculate DINOv2 objectness score and CLS embedding.
    ...
    if use_dinov2 and check_dinov2_available():
        try:
            ...
            for idx, _, basic_scores in candidates:
                ...
                objectness, embedding = calculate_dinov2_score(
                    frame_path, model, device
                )
                combined_score = (
                    0.5 * basic_scores["combined"]
                    + 0.5 * min(objectness / 10.0, 1.0)
                )
            ...
            print(
                f"DINOv2 enhancement failed: {e}, falling back to basic scoring"
            )
            use_dinov2 = False
